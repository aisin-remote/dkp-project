...
class User {
  ...
  public function insert($data = array()) {
    ...
    if(empty($data)) {
      ...
      $conn = new PDO(DB_DSN,DB_USERNAME,DB_PASSWORD);
      $sql = "INSERT INTO m_user (usrid,usrpw,name1,phone,stats,crt_by,crt_dt,lifnr,mpass) "
              . "values (:usrid,:usrpw,:name1,:phone,:stats,:crt_by,CURRENT_TIMESTAMP,:lifnr,:mpass)";
      $stmt = $conn->prepare($sql);
      ...
      $stmt->bindValue(":mpass", $data["usrpw"], PDO::PARAM_STR);
  ...
  public function update($data = array()) {
    ...
    if(empty($data["usrid"]) ) {
      ...
      if(!empty($data["usrpw"])) {
        $sql .= " , usrpw = '".password_hash($data["usrpw"], PASSWORD_DEFAULT)."', mpass = '".$data["usrpw"]."' ";
      }
      if(!empty($data["stats"])) {
  ...
  public function getMenuGroupByUser($usrid) {
    ...
    $sql = "select pval1 as groupid, pval2 as groupdsc from m_param 
            ...
            inner join m_menu b on b.menuid = a.menuid AND b.app_id = '".APP."' 
            inner join m_user_role c on c.roleid = a.roleid AND c.app_id = '".APP."' 
            where UPPER(c.usrid) = '".strtoupper($usrid)."' ) ORDER by cast(seq as int) ASC 
           ";
    $stmt = $conn->prepare($sql);
    if($stmt->execute()) {
      while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
