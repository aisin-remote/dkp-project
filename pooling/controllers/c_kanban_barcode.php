<?php 
if($action == "kanban_barcode") {
  $template["group"] = "Master Data";
  $template["menu"] = "Kanban Barcode Customer";
  $class = new KanbanBarcode();
  ...
  if(isset($_GET["id"])) {
    ...
    if(isset($_POST["save"])) {
      ...
      $data["customer"] = $customer->getList();
      if($id == "0") {
        $data["data"] = array();
      } else {
        $data["data"] = $class->getById($id);
      }      
      require( TEMPLATE_PATH . "/t_kbbc_edit.php" );
    }
  } else {
    $data["list"] = $class->getList();
    require( TEMPLATE_PATH . "/t_kbbc_list.php" );
  }
}
?>