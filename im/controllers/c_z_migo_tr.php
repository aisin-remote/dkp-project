...
if($action == "z_migo_tr_1") {
  ...
  $template["menu"] = "Stock Transfer MSTR to ASMP (Mounting Line)";
...
if($action == "z_migo_tr_2") {
  $template["group"] = "Z Transaction";
  $template["menu"] = "Stock Transfer MSTR to ASAN (Antenna Line)";
  $cMatDoc = new MaterialDocument();
  $cStock = new Stock();
  $cPlant = new Plant();
  $cSloc = new StoreLocation();
  ...
  $data["plants"] = $cPlant->getList();
  $data["materials"] = $cMara->getList();
  require( TEMPLATE_PATH . "/t_z_migo_tr_2.php" );
}
?>