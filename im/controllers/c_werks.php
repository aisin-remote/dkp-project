...
if ($action == "plant") {
  ...
  if (isset($_GET["id"])) {
    ...
    if (isset($_POST["save"])) {
      ...
      if ($save["status"] == true) {
        header("Location: ?action=" . $action . "&success=Plant%20Deleted");
      } else {
      ...
      require( TEMPLATE_PATH . "/t_werks_edit.php" );
    }
    ...
    if (!empty($activeSheetData)) {
      foreach ($activeSheetData as $row) {
        if ($i > 0) {
          ...
          $param["name1"] = trim($row["B"]);         
          $param["crt_by"] = $_SESSION[LOGIN_SESSION];
          $param["chg_by"] = $_SESSION[LOGIN_SESSION];

          if ($class->isExist($param["werks"])) {
            $save = $class->update($param);
          } else {
            $save = $class->insert($param);
          }
    ...
    require( TEMPLATE_PATH . "/t_werks_list.php" );
  }
}
?>