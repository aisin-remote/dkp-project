...
class Production
{
  ...
  public function getListLine()
  {
    ...
    $sql = "select a.* from m_prd_line a WHERE a.line_ty = 'ECU'";

    $sql .= " ORDER by a.seq::int ASC ";
  ...
  public function getListItemById($line, $date, $shift)
  {
    $return = array();
    $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);
    $sql = "SELECT a.*, TO_CHAR(a.prd_dt, 'YYYYMMDD') as xdate, CONCAT(b.mtart, ' - ', b.name1, ' (', b.backno ,')') as dies_name, 
  ...
  public function getItemById($line, $date, $shift, $seq)
  {
    ...
    $sql = "SELECT a.*, TO_CHAR(a.prd_dt, 'YYYYMMDD') as xdate, TO_CHAR(a.prd_dt, 'DD-MM-YYYY') as prod_date, CONCAT(x.mtart, ' ', x.name1) as dies_name, "
      ...
      . "LEFT JOIN m_prd_operator op5 ON op5.empid = h.op5id "
      . "LEFT JOIN m_prd_operator op6 ON op6.empid = h.op6id "
      . "LEFT JOIN m_prd_operator op7 ON op7.empid = h.op7id "
      . "LEFT JOIN m_prd_operator op8 ON op8.empid = h.op8id "
  ...
  public function insertStop($param)
  {
    ...
    if (empty($param)) {
      ...
      $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);
      $sql = "INSERT INTO t_prd_daily_stop (line_id, prd_dt, shift, prd_seq, stop_seq, start_time, end_time, stop_time, qty_stc, stop_id, action_id, exe_empid, remarks) "
        . "values (:line_id, :prd_dt, :shift, :prd_seq, (select coalesce(max(stop_seq),0)+1 as stop_seq FROM t_prd_daily_stop where line_id = :line_id2 AND prd_dt = :prd_dt2 AND shift = :shift2 AND prd_seq = :prd_seq2), :start_time, :end_time, :stop_time, :qty_stc, :stop_id, :action_id, :exe_empid, :remarks) ";
      $stmt = $conn->prepare($sql);
      $stmt->bindValue(":line_id", $param["line_id"], PDO::PARAM_STR);
      $stmt->bindValue(":prd_dt", $param["prd_dt"], PDO::PARAM_STR);
      $stmt->bindValue(":shift", $param["shift"], PDO::PARAM_STR);
