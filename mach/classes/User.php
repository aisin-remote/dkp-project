...
class User
{
  ...
  public function getById($usrid)
  {
    ...
    $sql = "SELECT * FROM mach.m_user a WHERE UPPER(a.usrid) = :usrid";
  ...
  public function getList()
  {
    $return = array();
    $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);
    $sql = "SELECT a.*, (select string_agg(roleid, ' , ') from mach.m_user_role where usrid = a.usrid and app_id = 'AISIN_MACH') as roles FROM mach.m_user a";
    $stmt = $conn->prepare($sql);
    if ($stmt->execute()) {
      while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
  ...
  public function insert($data = array())
  {
    ...
    if (empty($data)) {
      ...
      $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);
      $sql = "INSERT INTO mach.m_user (usrid,usrpw,name1,phone,stats,crt_by,crt_dt,lifnr) "
  ...
  public function update($data = array())
  {
    ...
    if (empty($data["usrid"])) {
      ...
      $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);
      $sql = "UPDATE mach.m_user SET usrid = '" . strtoupper($data["usrid"]) . "' ";
      if (!empty($data["name1"])) {
  ...
  public function insertUserRole($userid, $data)
  {
    ...
    if (empty($userid) || empty($data)) {
      ...
      $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);
      $sql = "INSERT INTO mach.m_user_role (usrid, roleid, app_id) VALUES ";
      $arr_insert = array();
      $userid = strtoupper(trim($userid));
      foreach ($data as $role) {
        $role = trim($role);
        $arr_insert[] = "('$userid','$role','AISIN_MACH')";
      }
  ...
  public function deleteUserRole($userid)
  {
    ...
    if (empty($userid)) {
      ...
      $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);
      $sql = "DELETE FROM mach.m_user_role WHERE UPPER(usrid) = '" . strtoupper($userid) . "' AND app_id = 'AISIN_MACH'";
      $stmt = $conn->prepare($sql);
      if ($stmt->execute()) {
        $return["status"] = true;
      } else {
  ...
  public function getMenuByUser($usrid)
  {
    ...
    $sql = "select distinct a.menuid, b.name1, b.groupid, b.sort1, c.usrid 
            from mach.m_role_menu a 
            inner join mach.m_menu b on b.menuid = a.menuid AND b.app_id = 'AISIN_MACH' 
            inner join mach.m_user_role c on c.roleid = a.roleid AND c.app_id = 'AISIN_MACH' 
            where UPPER(c.usrid) = '" . strtoupper($usrid) . "' AND a.app_id = 'AISIN_MACH' 
            ORDER by sort1 ASC";
    $stmt = $conn->prepare($sql);
    if ($stmt->execute()) {
      $menu_temp = null;
  ...
  public function getMenuGroupByUser($usrid)
  {
    $return = array();
    $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);
    $sql = "select pval1 as groupid, pval2 as groupdsc from mach.m_param 
            where pid = 'MENUGROUP' AND pval3 = 'AISIN_MACH' and pval1 in (select b.groupid 
            from mach.m_role_menu a 
            inner join mach.m_menu b on b.menuid = a.menuid AND b.app_id = 'AISIN_MACH' 
            inner join mach.m_user_role c on c.roleid = a.roleid AND c.app_id = 'AISIN_MACH' 
  ...
  public function getUserRole($id)
  {
    $return = array();
    $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);
    $sql = "SELECT * FROM mach.m_user_role WHERE UPPER(usrid) = :id and app_id = 'AISIN_MACH' ";
    // echo $sql;
    // echo $id;
    // die();
    $stmt = $conn->prepare($sql);
  ...
  public function isExist($usrid)
  {
    $return = false;
    $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);
    $sql = "SELECT count(*) as cnt FROM mach.m_user WHERE UPPER(usrid) = :id";
    $stmt = $conn->prepare($sql);
  ...
  public function updateStatus($extract_id)
  {
    $return = array();
    $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);
    $sql = "UPDATE mach.m_user SET stats = "
      . "(CASE "
      . "WHEN stats = 'A' THEN 'I' "
      . "WHEN stats = 'I' THEN 'A' "
      . "ELSE stats "
