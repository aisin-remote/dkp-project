...
if ($action == "api_get_time_stop") {
  ...
  $sql = "select * from mach.m_prd_shift_stop where srna_id = '$srna_id' and shift_id = '$shift' AND time_id = '$time_id' LIMIT 1";
...
if ($action == "api_approve_daily_i") {
  ...
  $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);
  $query = "UPDATE mach.t_prd_daily_i SET stats = 'A', apr_by = '$apr_by', apr_dt = CURRENT_TIMESTAMP WHERE line_id = '$line_id' AND prd_dt = '$prd_dt' AND shift = '$shift' AND prd_seq = '$prd_seq'";
...
if ($action == "api_approve_daily_h") {
  ...
  $apr_by = $_SESSION[LOGIN_SESSION];
  $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);
  $query = "UPDATE mach.t_prd_daily_h SET apr_spv_by = '$apr_by', apr_spv_dt = CURRENT_TIMESTAMP WHERE line_id = '$line_id' AND TO_CHAR(prd_dt, 'YYYYMMDD') = '$prd_dt' AND shift = '$shift'";
  $stmt = $conn->prepare($query);
  if ($stmt->execute()) {
    $return["status"] = true;
  } else {
...
if ($action == "api_get_qty") {
  ...
  // inner join m_prd_line b ON b.line_id = a.line_id and b.line_ty = 'MCH'
  ...
  (select sum(ng_qty) from mach.t_prd_daily_ng where line_id = a.line_id and prd_dt = a.prd_dt and prd_seq = a.prd_seq) as ng_qty 
