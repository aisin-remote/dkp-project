...
if ($action == "api_dashboard_prd") {
  ...
  if (empty($data_line_name)) {
    $query = "SELECT name1 FROM mach.m_prd_line ORDER by line_id ASC";
    $stmt = $conn->prepare($query);
    if ($stmt->execute()) {
      $i = 0;
      while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
  ...
  $query_sum = "select line_id, line_name, avg(cctime) as cctime, sum(pln_qty) as pln_qty, sum(prd_time) as prd_time, sum(prd_qty) as prd_qty, sum(ril_qty) as ril_qty, sum(rol_qty) as rol_qty, sum(per_jam) as per_jam from ( 
    ...
    (select coalesce(sum(ng_qty),0) from mach.t_prd_daily_ng 
    ...
    inner join mach.m_prd_line b ON b.line_id = a.line_id 
	  INNER JOIN mach.m_param p ON p.pid = 'SHIFT' AND a.shift = p.seq 
    LEFT JOIN ( 
      SELECT line_id, line_ty 
...
if ($action == "api_dashboard_prd_single") {
  ...
  $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);
  $query = "SELECT name1 FROM mach.m_prd_line WHERE line_id = '$line_id'";
  $stmt = $conn->prepare($query);
  ...
  $query = "select a.line_id, b.name1 as line_name, a.cctime, a.pln_qty, a.prd_time, coalesce(a.prd_qty,0) as prd_qty, 
            ...
            from mach.t_prd_daily_i a
            inner join mach.t_prd_daily_stop b on b.line_id = a.line_id and b.prd_dt = a.prd_dt and b.shift = a.shift and b.prd_seq = a.prd_seq
            inner join mach.m_prd_stop_reason_action c on c.srna_id = b.stop_id and c.type3 = 'MESIN'
            ...
            from mach.t_prd_daily_i a
            inner join mach.t_prd_daily_stop b on b.line_id = a.line_id and b.prd_dt = a.prd_dt and b.shift = a.shift and b.prd_seq = a.prd_seq
            inner join mach.m_prd_stop_reason_action c on c.srna_id = b.stop_id and c.type3 = 'DIES'
            where a.line_id = '$line_id' AND a.prd_dt = '$today' and TO_CHAR(TO_TIMESTAMP(a.prd_dt||' '||a.time_end,'YYYY-MM-DD HH24:MI'),'HH24') = '$jam_end' 
...
if ($action == "dashboard_line") {
  ...
  $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);
  $line_name = "???";
  $query_line = "SELECT * FROM mach.m_prd_line WHERE line_ty = 'MCH' ORDER BY line_id asc";
